...
namespace WindTurbineVR
{
    public class HarnessController : MonoBehaviour
    {
        ...
        void Update()
        {
            transform.parent.position = mainCamera.position + new Vector3(0, -0.3f, 0);
            transform.parent.rotation = Quaternion.Euler(transform.parent.rotation.eulerAngles.x, mainCamera.rotation.eulerAngles.y + 90, transform.parent.rotation.eulerAngles.z);
        }
